        ...
        <>
            <div className="relative bg-font-dark-purple w-full h-50 top-0">
                ...
                <div className="absolute left-60 -bottom-17 flex gap-1 font-medium items-baseline">
                    <a href={`mailto:${jobSeekerInfo?.personalInfoDetailsReadOnlyDTO.email}`} className="flex items-center gap-1"><Mail strokeWidth={1.25}  />{jobSeekerInfo?.personalInfoDetailsReadOnlyDTO.email}</a>
                    {jobSeekerInfo?.personalInfoDetailsReadOnlyDTO.telephoneNumber && (
                        <div className="flex items-baseline gap-1">
                            <Dot/>
                            <span className="flex gap-1">
                                <Phone strokeWidth={1.25} />{jobSeekerInfo?.personalInfoDetailsReadOnlyDTO.telephoneNumber}
                            </span>
                        </div>
                    )}
                </div>
